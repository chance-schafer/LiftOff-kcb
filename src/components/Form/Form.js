...
export const FormElement = () => {
  ...
  const validateForm = () => {
    ...
    const newErrors = {};
    // ---------------------- Business Name ----------------------
    if (!businessName || businessName === "" || /^[\s]*$/.test(businessName))
      newErrors.businessName = "Please enter your Business Name.";
    // Not empty & no whitespace
    else if (businessName.length < 2 || businessName.length > 20)
      newErrors.businessName =
        "Business Name must be between 2-20 characters in length.";
    // Checks length
    else if (!/^[a-zA-Z\s]+$/.test(businessName))
      newErrors.businessName =
        "Please remove any special characters or numbers."; // No special char's or #'s

    // ---------------------- Business Type ----------------------
    if (!businessTypes || businessTypes === "" || /^[\s]*$/.test(businessTypes))
      newErrors.businessTypes = "Please select or enter your Business Type.";
    else if (businessTypes.length < 2 || businessTypes.length > 20)
      newErrors.businessTypes =
        "Business Type must be between 2-20 characters in length.";
    else if (!/^[a-zA-Z\s&]+$/.test(businessTypes))
      newErrors.businessTypes =
        "Please remove any special characters or numbers.";

    // ---------------------- County ----------------------
    if (!businessCounty || businessCounty === "")
      newErrors.businessCounty = "Please select a County.";
    else if (!/^[A-Z][a-z]/.test(businessCounty))
      newErrors.businessCounty =
        "Please Capitalize the first letter in the County name.";

    // ---------------------- City ----------------------
    if (!businessCity || businessCity === "" || /^[\s]*$/.test(businessCity))
      newErrors.businessCity = "Please select a City.";
    // Not empty & no whitespace
    else if (businessCity.length < 2 || businessCity.length > 20)
      newErrors.businessCity =
        "City must be between 2-20 characters in length.";
    // Checks length
    else if (!/^[a-zA-Z\s']+$/.test(businessCity))
      newErrors.businessCity =
        "Please remove any special characters or numbers."; // No special char's or #'s (except: ')
    ...
    // ---------------------- Zip Code ----------------------
    if (!businessZipCode || businessZipCode === "")
      newErrors.businessZipCode = "Please enter your Zip Code.";
    else if (!/^\d+$/.test(businessZipCode))
      newErrors.businessZipCode = "Please remove any special characters.";

    // ---------------------- Street & Number ----------------------
    if (
      !businessStreetNumber ||
      businessStreetNumber === "" ||
      /^[\s]*$/.test(businessStreetNumber)
    )
      newErrors.businessStreetNumber =
        "Please enter your Business street name and number.";
    else if (
      businessStreetNumber.length < 2 ||
      businessStreetNumber.length > 20
    )
      newErrors.businessStreetNumber =
        "This Field must be between 2-20 characters in length.";

    // ---------------------- Description ----------------------
    if (
      !businessDescription ||
      businessDescription === "" ||
      /^[\s]*$/.test(businessDescription)
    )
      ...
      newErrors.businessDescription =
        "Description must be between 10-160 characters in length.";

    // ---------------------- Web Link ----------------------
    if (
      !businessWebsiteLink ||
      businessWebsiteLink === "" ||
      /^[\s]*$/.test(businessWebsiteLink)
    )
      newErrors.businessWebsiteLink =
        "Please enter your business website link.";
    else if (businessWebsiteLink.length < 2 || businessWebsiteLink.length > 100)
      newErrors.businessWebsiteLink =
      ...
      newErrors.businessWebsiteLink = "Please enter a valid webiste format.";

    // ---------------------- Identity ----------------------
    if (!checkInputs.includes(true))
      newErrors.businessIdentity = "Please select at least one options.";
  ...
  return (
    ...
    >
      ...
      >
        <path
          d="M87.3886 864.659L54.5953 43.3376L684.654 2.6567L730.866 778.829L87.3886 864.659Z"
          stroke="#AA283C"
          strokeWidth="5"
          strokeLinejoin="round"
        />
        <path
          d="M35.3886 905.659L2.59533 84.3376L632.654 43.6567L678.866 819.829L35.3886 905.659Z"
          stroke="#201D1D"
          strokeWidth="5"
          strokeLinejoin="round"
        />
      ...
      <Form id="form">
          ...
          <Row className="mt-1">
            <Col>
              <FloatingLabel
                ...
                controlId="businessStreetNumber"
              >
                <FormControl
                  id="address-description"
                  type="text"
                  onChange={(e) =>
                    setField("businessStreetNumber", e.target.value)
                  }
                  onFocus={(e) =>
                    setField("businessStreetNumber", e.target.value)
                  }
        ...
        >
          ...
          <Form.Text className="text-danger">
            {!!errors.businessIdentity && <div>{errors.businessIdentity}</div>}
          </Form.Text>
        </FormGroup>
            ...
            <FormGroup className="mb-3 input" controlId="businessWebsiteLink">
              <FloatingLabel label="Website link">
                <FormControl
                  ...
                  type="text"
                  onChange={(e) =>
                    setField("businessWebsiteLink", e.target.value)
                  }
                  onFocus={(e) =>
                    setField("businessWebsiteLink", e.target.value)
                  }
