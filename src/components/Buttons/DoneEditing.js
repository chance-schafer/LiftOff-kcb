...
export const DoneEditing = () => {
  ...
  const onClickEdit = () => {
    ...
    const addressDescription = document.getElementById(
      "address-description"
    ).value;
    ...
    const updateInfo = async () => {
      try {
        const response = await fetch(
          `https://liftoff-kcb-backend-production-bb79.up.railway.app/api/businesses/${businessToEdit.id}`,
          {
            headers: {
