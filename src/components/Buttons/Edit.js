  ...
  const onClick = () => {
    ...
    const getBusinessToEdit = async () => {
      try {
        const response = await fetch(
          `https://liftoff-kcb-backend-production-bb79.up.railway.app/api/businesses/${id}`,
          {
            headers: {
    ...
    getBusinessToEdit().then((response) => {
      ...
      document.getElementById("business-type").value =
        response.businessType.name;
      document.getElementById("county").value =
        response.businessLocation.county;
      document.getElementById("business-city").value =
        response.businessLocation.city;
      document.getElementById("zipcode").value =
        response.businessLocation.zipCode;
      // document.getElementById('address-description').value;
      document.getElementById("description").value =
        response.businessDetails.description;
      document.getElementById("website").value =
        response.businessDetails.websiteUrl;
